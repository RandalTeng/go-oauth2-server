...
func NewDefaultServer(manager definition.Manager) *Server {
	return NewServer(NewConfig(), manager)
}

// NewServer create authorization server
func NewServer(cfg *Config, manager definition.Manager) *Server {
	srv := &Server{
		Config:  cfg,
		Manager: manager,
	}
...
func (s *Server) CheckResponseType(rt definition.ResponseType) bool {
...
func (s *Server) CheckCodeChallengeMethod(ccm definition.CodeChallengeMethod) bool {
...
func (s *Server) ValidationAuthorizeRequest(r *http.Request) (*AuthorizeRequest, error) {
	...
	resType := definition.ResponseType(r.FormValue("response_type"))
	if resType.String() == "" {
	...
	ccm := definition.CodeChallengeMethod(r.FormValue("code_challenge_method"))
	// set default
	if ccm == "" {
...
func (s *Server) GetAuthorizeToken(ctx context.Context, req *AuthorizeRequest) (any, error) {
	...
	if fn := s.ClientAuthorizedHandler; fn != nil {
		...
		if req.ResponseType == definition.Token {
			gt = definition.Implicit
		}
...
func (s *Server) ValidationTokenRequest(r *http.Request) (definition.GrantType, *definition.TokenGenerateRequest, error) {
	if v := r.Method; !(v == "POST" ||
		(s.Config.AllowGetAccessRequest && v == "GET")) {
		return "", nil, errors.ErrInvalidRequest
	}

	gt := definition.GrantType(r.FormValue("grant_type"))
	if gt.String() == "" {
...
func (s *Server) CheckGrantType(gt definition.GrantType) bool {
...
func (s *Server) GetAccessToken(
	...
	case definition.AuthorizationCode:
		...
		return ti, nil
	case definition.PasswordCredentials, definition.ClientCredentials:
		if fn := s.ClientScopeHandler; fn != nil {
			allowed, err := fn(tgr)
			if err != nil {
				return nil, err
...
func (s *Server) ValidationBearerToken(r *http.Request) (definition.TokenInfo, error) {
	ctx := r.Context()
